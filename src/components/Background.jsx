...
const AnimatedBackground = () => {
	...
	// Initial positions for each blob
	...
	useEffect(() => {
		...
		const handleScroll = () => {
			...
			// Move each blob based on scroll
			blobRefs.current.forEach((blob, index) => {
				const initialPos = initialPositions[index];

				// Movement calculations
				const xOffset = Math.sin(newScroll / 100 + index * 0.5) * 340; // Horizontal shift
				const yOffset = Math.cos(newScroll / 100 + index * 0.5) * 40;  // Vertical shift
